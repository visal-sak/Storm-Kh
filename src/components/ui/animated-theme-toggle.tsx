...
export const AnimatedThemeToggle = ({ className }: { className?: string }) => {
  // Theme lives in the app-wide provider, not local state, so the button stays
  // in sync with the basemap, the tooltips and the persisted preference.
  ...
  return (
    <Button
      onClick={toggleTheme}
      // Square icon button: same 36px box as the text buttons beside it.
...
const SolarSwitch = ({ isDark }: { isDark: boolean }) => {
  ...
  // Each icon's stroke is drawn on as it scales up: pathLength is derived from
  // the same motion value the variants animate, so the rays draw themselves.
  ...
  return (
    <motion.div animate={isDark ? "checked" : "unchecked"} aria-hidden="true">
      <motion.svg
        width="16"
        height="16"
        viewBox="0 0 25 25"
      ...
      >
        <motion.path
          ...
          {...sunPath}
        />
        <motion.path d="M12.4058 1.76251V3.76251" {...sunPath} />
        <motion.path d="M12.4058 21.7625V23.7625" {...sunPath} />
        <motion.path d="M4.62598 4.98248L6.04598 6.40248" {...sunPath} />
        <motion.path d="M18.7656 19.1225L20.1856 20.5425" {...sunPath} />
        <motion.path d="M1.40576 12.7625H3.40576" {...sunPath} />
        <motion.path d="M21.4058 12.7625H23.4058" {...sunPath} />
        <motion.path d="M4.62598 20.5425L6.04598 19.1225" {...sunPath} />
        <motion.path d="M18.7656 6.40248L20.1856 4.98248" {...sunPath} />
        <motion.path
